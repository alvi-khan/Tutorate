...
export const EditTutorProfileForm = (props) => {
    ...
    const editProfile = async (event) => {
        ...
        handleClose();
        const res = await fetch(`${process.env.REACT_APP_BASE_URL}/tutor/edit`, {
            method: 'POST',
            credentials: 'include',
            body: formData,
        });
        ...
        >
            ...
            <form onSubmit={editProfile} className="optionsContainer">
                <Grid container spacing={4} alignItems="center">
                    <Grid item xs={6}>
                        <ImageUpload image={user.tutor.image === null ? image : undefined} setImage={setImage} imageLink={user.tutor.image === null ? "" : `${process.env.REACT_APP_BASE_URL}${user.tutor.image}`}/>
                    </Grid>
                    <Grid item xs={6}>
